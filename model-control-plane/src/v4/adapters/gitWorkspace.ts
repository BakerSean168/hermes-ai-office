...
function executionComponent(value: string): string {
  ...
  if (!component || component.length > MAX_IDENTIFIER || !/^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(component)) {
...
function decodeTest(value: unknown): TestCommandEvidence {
  ...
  if (status !== 'PASS' && status !== 'FAIL' && status !== 'SKIP') throw new V4Error('WORKSPACE_TEST_EVIDENCE_INVALID');
  const exitCode = item.exitCode;
  if (exitCode !== undefined && (!Number.isInteger(exitCode) || Number(exitCode) < 0)) throw new V4Error('WORKSPACE_TEST_EVIDENCE_INVALID');
  return {
...
function decodeImplementationEvidence(value: unknown): ImplementationCompletionEvidence {
  ...
  if (root.version !== 1 || (root.phase !== 'IMPLEMENT' && root.phase !== 'IMPLEMENT_FIX')) throw new V4Error('WORKSPACE_IMPLEMENTATION_EVIDENCE_INVALID');
  if (!Array.isArray(root.tests)) throw new V4Error('WORKSPACE_TEST_EVIDENCE_INVALID');
  const tests = root.tests.map(decodeTest);
  return {
    version: 1,
    executionId: requiredString(root.executionId, 'WORKSPACE_EVIDENCE_EXECUTION_REQUIRED', MAX_IDENTIFIER),
    phase: root.phase,
    sourceRevision: requiredString(root.sourceRevision, 'WORKSPACE_EVIDENCE_SOURCE_REQUIRED', MAX_IDENTIFIER),
    resultRevision: requiredString(root.resultRevision, 'WORKSPACE_EVIDENCE_RESULT_REQUIRED', MAX_IDENTIFIER),
...
function decodeReviewEvidence(value: unknown): ReviewCompletionEvidence {
  const root = record(value, 'WORKSPACE_EVIDENCE_INVALID');
  if (root.version !== 1 || root.phase !== 'REVIEW') throw new V4Error('WORKSPACE_REVIEW_EVIDENCE_INVALID');
  if (root.verdict !== 'PASS' && root.verdict !== 'FAIL' && root.verdict !== 'INVALID') throw new V4Error('WORKSPACE_REVIEW_VERDICT_INVALID');
  if (!Array.isArray(root.findings) || !root.findings.every((item) => typeof item === 'string' && item.length <= 8_000)) {
...
function descriptorFrom(value: unknown): StoredWorkspaceDescriptor {
  ...
  const descriptor: StoredWorkspaceDescriptor = {
    ...
    evidenceExecutionPath: requiredString(root.evidenceExecutionPath, 'WORKSPACE_DESCRIPTOR_INVALID'),
    sourceRepositoryPath: requiredString(root.sourceRepositoryPath, 'WORKSPACE_DESCRIPTOR_INVALID'),
    sourceRevision: requiredString(root.sourceRevision, 'WORKSPACE_DESCRIPTOR_INVALID', MAX_IDENTIFIER),
...
export class LocalGitWorkspaceAdapter implements WorkspaceProviderPort {
  ...
  constructor(options: LocalGitWorkspaceOptions) {
    ...
    this.allowedRoots = options.allowedRepositoryRoots.map((root) => this.canonicalDirectory(root, 'WORKSPACE_ALLOWED_ROOT_INVALID'));
    this.managedHostRoot = this.canonicalDirectory(options.managedHostRoot, 'WORKSPACE_MANAGED_ROOT_INVALID');
  ...
  async provision(input: WorkspaceProvisionInput): Promise<WorkspaceDescriptor> {
    ...
    failClosed(input.sourceRevision.trim().length > 0 && input.sourceRevision.length <= MAX_IDENTIFIER, 'WORKSPACE_SOURCE_REVISION_REQUIRED');
    ...
    const executionPath = path.posix.join(this.executionRoot, 'v4', 'executions', executionId, 'repo');
    const evidenceExecutionPath = path.posix.join(this.executionRoot, 'v4', 'executions', executionId, 'completion-evidence.json');
    ...
    if (fs.existsSync(executionDirectory)) {
      ...
      for (const [key, value] of Object.entries(expected)) {
        if (existing[key as keyof typeof existing] !== value) throw new V4Error('WORKSPACE_PROVENANCE_MISMATCH');
      }
      this.assertManagedWorkspace(existing.hostPath);
      if (!(await this.gitSucceeds(existing.hostPath, ['cat-file', '-e', input.sourceRevision + '^{commit}']))) {
    ...
    if (input.sourceWorkspace) {
      ...
      if (source.sourceRepositoryPath !== repositoryRoot) throw new V4Error('WORKSPACE_SOURCE_REPOSITORY_MISMATCH');
      if (!(await this.gitSucceeds(source.hostPath, ['cat-file', '-e', input.sourceRevision + '^{commit}']))) {
        throw new V4Error('WORKSPACE_SOURCE_REVISION_MISSING');
      }
      cloneSource = source.hostPath;
    } else if (!(await this.gitSucceeds(repositoryRoot, ['cat-file', '-e', input.sourceRevision + '^{commit}']))) {
      throw new V4Error('WORKSPACE_SOURCE_REVISION_MISSING');
    }
    ...
    try {
      ...
      await this.runGit(['clone', '--no-hardlinks', '--no-checkout', '--', cloneSource, stagingRepo]);
      if (input.phase === 'REVIEW') {
        await this.git(stagingRepo, ['checkout', '--detach', input.sourceRevision]);
      } else {
        await this.git(stagingRepo, ['switch', '-c', 'pixel-v4/' + executionId, input.sourceRevision]);
      }
  ...
  async verifyImplementation(workspace: WorkspaceDescriptor): Promise<WorkspaceCompletionSnapshot> {
    ...
    const headRevision = await this.git(descriptor.hostPath, ['rev-parse', '--verify', 'HEAD^{commit}']);
    if (headRevision === descriptor.sourceRevision) throw new V4Error('WORKSPACE_IMPLEMENTATION_NOOP');
    const descendantOfSource = await this.gitSucceeds(descriptor.hostPath, ['merge-base', '--is-ancestor', descriptor.sourceRevision, headRevision]);
    ...
    if (evidence.executionId !== descriptor.executionId || evidence.sourceRevision !== descriptor.sourceRevision || evidence.resultRevision !== headRevision) {
      throw new V4Error('WORKSPACE_IMPLEMENTATION_EVIDENCE_MISMATCH');
    }
    if (evidence.tests.length === 0 || evidence.tests.some((item) => item.status === 'FAIL') || !evidence.tests.some((item) => item.status === 'PASS')) {
      throw new V4Error('WORKSPACE_IMPLEMENTATION_TEST_GATE_FAILED');
    }
    const changedFilesRaw = await this.git(descriptor.hostPath, ['diff', '--name-only', '-z', descriptor.sourceRevision + '..' + headRevision]);
    const changedFiles = changedFilesRaw.split('\0').filter(Boolean);
    if (changedFiles.length === 0 || changedFiles.length > MAX_CHANGED_FILES) throw new V4Error('WORKSPACE_CHANGED_FILES_INVALID');
    const diffStat = await this.git(descriptor.hostPath, ['diff', '--stat', '--summary', descriptor.sourceRevision + '..' + headRevision]);
    if (Buffer.byteLength(diffStat, 'utf8') > MAX_DIFF_STAT_BYTES) throw new V4Error('WORKSPACE_DIFF_STAT_TOO_LARGE');
    return {
  ...
  async verifyReview(workspace: WorkspaceDescriptor, reviewedSha: string): Promise<WorkspaceCompletionSnapshot> {
    ...
    if (!clean) throw new V4Error('WORKSPACE_DIRTY');
    const headRevision = await this.git(descriptor.hostPath, ['rev-parse', '--verify', 'HEAD^{commit}']);
    if (headRevision !== reviewedSha || descriptor.sourceRevision !== reviewedSha) throw new V4Error('WORKSPACE_REVIEW_SHA_MISMATCH');
    const evidence = decodeReviewEvidence(this.readEvidence(descriptor.evidenceHostPath));
    if (evidence.executionId !== descriptor.executionId || evidence.reviewedSha !== reviewedSha) throw new V4Error('WORKSPACE_REVIEW_EVIDENCE_MISMATCH');
    if (evidence.verdict === 'PASS') {
      if (evidence.checks.length === 0 || evidence.checks.some((item) => item.status === 'FAIL') || !evidence.checks.some((item) => item.status === 'PASS')) {
        throw new V4Error('WORKSPACE_REVIEW_CHECK_GATE_FAILED');
      }
    } else if (evidence.findings.length === 0 && !evidence.checks.some((item) => item.status === 'FAIL')) {
  ...
  async integrateAcceptedRevision(input: {
    ...
    if (candidate.sourceRepositoryPath !== repositoryRoot) throw new V4Error('WORKSPACE_SOURCE_REPOSITORY_MISMATCH');
    const candidateClean = (await this.git(candidate.hostPath, ['status', '--porcelain=v1', '-z'])).length === 0;
    const candidateHead = await this.git(candidate.hostPath, ['rev-parse', '--verify', 'HEAD^{commit}']);
    if (!candidateClean || candidateHead !== input.acceptedRevision) throw new V4Error('WORKSPACE_ACCEPTED_REVISION_NOT_PINNED');
    if (!(await this.gitSucceeds(candidate.hostPath, ['merge-base', '--is-ancestor', input.expectedRevision, input.acceptedRevision]))) {
      throw new V4Error('WORKSPACE_ACCEPTED_REVISION_NOT_DESCENDANT');
    }
    const gitDirectory = path.resolve(repositoryRoot, await this.git(repositoryRoot, ['rev-parse', '--git-dir']));
    ...
    try {
      ...
      if ((await this.git(repositoryRoot, ['status', '--porcelain=v1', '-z'])).length !== 0) throw new V4Error('WORKSPACE_INTEGRATION_DIRTY');
      await this.git(repositoryRoot, ['fetch', '--no-tags', '--', candidate.hostPath, input.acceptedRevision]);
      if (!(await this.gitSucceeds(repositoryRoot, ['merge-base', '--is-ancestor', input.expectedRevision, input.acceptedRevision]))) {
        throw new V4Error('WORKSPACE_ACCEPTED_REVISION_NOT_DESCENDANT');
      }
      const beforeMerge = await this.git(repositoryRoot, ['rev-parse', '--verify', 'HEAD^{commit}']);
      if (beforeMerge !== input.expectedRevision) throw new V4Error('WORKSPACE_INTEGRATION_STALE_HEAD');
      ...
      if (after !== input.acceptedRevision) throw new V4Error('WORKSPACE_INTEGRATION_RESULT_MISMATCH');
      if ((await this.git(repositoryRoot, ['status', '--porcelain=v1', '-z'])).length !== 0) throw new V4Error('WORKSPACE_INTEGRATION_DIRTY');
  ...
  private async repositoryRoot(repositoryPath: string): Promise<string> {
    ...
    if (!this.allowedRoots.some((root) => inside(lexical, root))) throw new V4Error('WORKSPACE_REPOSITORY_NOT_ALLOWED');
    ...
    if (!this.allowedRoots.some((allowed) => inside(real, allowed))) throw new V4Error('WORKSPACE_REPOSITORY_NOT_ALLOWED');
    return real;
  }
  ...
  private assertManagedDirectory(directory: string): void {
    ...
    const stat = fs.lstatSync(lexical, { throwIfNoEntry: false });
    if (!stat?.isDirectory() || stat.isSymbolicLink() || fs.realpathSync(lexical) !== lexical) throw new V4Error('WORKSPACE_PATH_NOT_MANAGED');
  }

  private assertManagedWorkspace(hostPath: string): void {
    this.assertManagedDirectory(hostPath);
    if (!inside(hostPath, path.join(this.managedHostRoot, 'v4', 'executions'))) throw new V4Error('WORKSPACE_PATH_NOT_MANAGED');
    const gitEntry = fs.lstatSync(path.join(hostPath, '.git'), { throwIfNoEntry: false });
    if (!gitEntry?.isDirectory() || gitEntry.isSymbolicLink()) throw new V4Error('WORKSPACE_GIT_MISSING');
  }
  ...
  private validateWorkspace(value: WorkspaceDescriptor): WorkspaceDescriptor {
    ...
    const sourceRepositoryPath = this.canonicalDirectory(descriptor.sourceRepositoryPath, 'WORKSPACE_SOURCE_REPOSITORY_INVALID');
    if (sourceRepositoryPath !== descriptor.sourceRepositoryPath || !this.allowedRoots.some((root) => inside(sourceRepositoryPath, root))) {
    ...
    if (descriptor.hostPath !== path.join(expectedDirectory, 'repo') || descriptor.evidenceHostPath !== path.join(expectedDirectory, 'completion-evidence.json')) {
      throw new V4Error('WORKSPACE_PROVENANCE_MISMATCH');
    }
    if (descriptor.executionPath !== path.posix.join(this.executionRoot, 'v4', 'executions', executionId, 'repo')) throw new V4Error('WORKSPACE_PROVENANCE_MISMATCH');
    if (descriptor.evidenceExecutionPath !== path.posix.join(this.executionRoot, 'v4', 'executions', executionId, 'completion-evidence.json')) throw new V4Error('WORKSPACE_PROVENANCE_MISMATCH');
    const stored = this.readDescriptor(path.join(expectedDirectory, 'workspace.json'));
    if (JSON.stringify(stored) !== JSON.stringify(descriptor)) throw new V4Error('WORKSPACE_DESCRIPTOR_MISMATCH');
    return descriptor;
  }

  private readDescriptor(file: string): StoredWorkspaceDescriptor {
    const stat = fs.lstatSync(file, { throwIfNoEntry: false });
    if (!stat?.isFile() || stat.isSymbolicLink() || stat.size > 64_000) throw new V4Error('WORKSPACE_DESCRIPTOR_INVALID');
    try {
  ...
  private readEvidence(file: string): Record<string, unknown> {
    const stat = fs.lstatSync(file, { throwIfNoEntry: false });
    if (!stat?.isFile() || stat.isSymbolicLink() || stat.size <= 0 || stat.size > MAX_EVIDENCE_BYTES) throw new V4Error('WORKSPACE_EVIDENCE_INVALID');
    try {
  ...
  private async git(cwd: string, args: string[]): Promise<string> {
    return await this.runGit(['-c', 'core.fsmonitor=false', '-c', 'core.hooksPath=/dev/null', '-c', 'safe.directory=' + cwd, '-C', cwd, ...args]);
  }

  private async gitOptional(cwd: string, args: string[]): Promise<string | undefined> {
    try { return await this.git(cwd, args); } catch { return undefined; }
  }
