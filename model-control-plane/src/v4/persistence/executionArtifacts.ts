...
function sessionFrom(row: SessionRow): ExecutionSession {
  ...
  if (!isProviderStatus(row.provider_status))
    throw new V4Error('CORRUPTED_PROVIDER_SESSION_STATUS');
  if (
  ...
  if (
    ...
    throw new V4Error('CORRUPTED_EXECUTION_SESSION_RESULT');
  ...
  create(input: {
    ...
    return withTransaction(this.db, () => {
      ...
      if (execution.source_revision !== input.sourceRevision)
        throw new V4Error('SESSION_SOURCE_REVISION_MISMATCH');
      if (execution.repository_path !== input.workspace.sourceRepositoryPath)
        throw new V4Error('SESSION_REPOSITORY_MISMATCH');
  ...
  attachProviderSession(
    ...
    return withTransaction(this.db, () => {
      ...
      if (TERMINAL_PROVIDER_STATUSES.has(current.providerStatus))
        throw new V4Error('EXECUTION_SESSION_TERMINAL');
  ...
  complete(
    ...
    return withTransaction(this.db, () => {
      ...
      if (!PROVIDER_TRANSITIONS[current.providerStatus].includes(input.status))
        throw new V4Error('INVALID_PROVIDER_SESSION_TRANSITION');
