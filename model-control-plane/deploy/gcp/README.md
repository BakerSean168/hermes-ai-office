...
V4 also owns a separate host-level high-watermark guard for Docker/build cache. `hermes-pixel-v4-host-cache.timer` runs every 15 minutes but does nothing while free space is at least 16 GiB. Below that trigger it refuses destructive work while a Pixel Execution is `RUNNING` or a formal V4 release owns the shared lock, then escalates only through aged BuildKit cache, all unused BuildKit cache, dangling images, and unused images older than seven days until the 24 GiB target is reached. It never runs `docker system prune`, never automatically prunes Docker volumes, and never deletes images referenced by a container. The root-only maintenance record is projected through `/api/health` and `/api/v4/storage` as `hostCacheMaintenance`; malformed state is reported as `INVALID` rather than echoed.

Formal V4 releases use an explicit exact-SHA approval ref rather than the mutable canonical working tree. After the required review/quality gates have accepted a commit, pin it with `model-control-plane/scripts/pin-v4-release-ref.sh <sha>`. The ref `refs/pixel-v4/release-approved` is fast-forward-only and reflogged. `release-v4-gcp.sh` then creates a detached transient worktree at that exact SHA, verifies it shares the canonical Git common directory and that the bootstrap launcher matches the approved commit, runs the complete test/build/smoke gate from the transient tree, atomically exchanges the compiled artifact, and removes the release worktree. Unrelated dirty files in the developer checkout therefore do not require stash/reset or HEAD movement.

Writer completion persists the exact Git HEAD returned by the clean-descendant completion verifier as durable `result_revision` provenance; batch-only integration and later repair ancestry gates use that result revision rather than confusing it with the writer's source/base revision. Executions created before this provenance column existed remain fail-closed. If a recoverable Plan is blocked specifically with `BATCH_INTEGRATION_EVIDENCE_MISSING`, an operator may POST exact legacy evidence to `/api/v3/development/plans/:planId/legacy-result-revisions`; the control plane accepts only successful non-system writer executions in the blocked batch, re-runs the normal completion verifier against the preserved managed workspace, requires the verified HEAD to equal the operator-attested 40-hex revision, records an `OPERATOR_SUBMITTED` provenance event, and then enters the normal AUTO recovery path. This endpoint only repairs missing provenance: it does not mark a work item or batch successful, create an integration ref, or bypass the independent batch review. New executions must never depend on this compatibility path.

Antigravity is a deliberate provider-native exception to the LiteLLM-managed model path. It is registered as an opt-in external adapter for **trusted task input only**; ordinary task routing is unchanged, while `EXTERNAL_CHANGE` plans reject both Antigravity backends before persistence (`untrusted_external: false`) and use the normal Codex/Claude/OpenHands governance path. Review currently uses `gemini-3.1-pro-high`; implementation should prefer `gemini-3.8-flash-high` with `gemini-3.7-flash-high` as a compatibility fallback. GCP `agy` 1.1.22 advertises both and a live authenticated 3.8 Flash High print-mode smoke passed on 2026-09-03. Both Antigravity backends nevertheless remain disabled at runtime until explicitly added to `MODEL_CP_V3_ENABLED_BACKENDS`, and the V4 path still needs the readiness adapter wired to a provider-native execution provider. The adapter sends objectives over stdin using Antigravity stream-JSON, constrains review results with JSON Schema, stores only durable execution metadata/output, and runs the CLI as the authenticated `dev` identity inside private mount and PID namespaces that hide unrelated homes and sibling AI Office workspaces. The wrapper copies only the minimum consumer-auth/config files into short-lived private tmpfs state, so host auth, prior conversations, brain state, and caches are not mounted writable and sibling workspaces cannot access them. Current `agy` keeps provider auth and model-controlled file tools in the same process; adversarial probes confirmed those tools can read the private OAuth copy, so provider-native Antigravity is intentionally **not** considered secret-safe for untrusted PR/repository input. Native launches require the configured consumer home to live strictly below `/home`, use a fixed `/bin/bash`, a root-safe PATH, `unshare --pid --fork --kill-child=SIGKILL --mount-proc`, and an OpenHands-compatible shared workspace GID; the root control plane re-normalizes terminal writer trees to group-writable permissions before handoff. A real `setsid` escape smoke confirmed delayed descendants do not survive cancellation.
