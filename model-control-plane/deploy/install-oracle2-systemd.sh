...
if [[ -f "$runtime_env" ]]; then
  ...
  echo "warning: $runtime_env is absent; LiteLLM Admin URL metadata will be omitted" >&2
  echo "copy deploy/model-control-plane-v3.env.example and set the host-specific URL if desired" >&2
fi
