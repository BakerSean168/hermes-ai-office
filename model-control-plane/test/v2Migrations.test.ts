...
test('V2 schema migration is additive and idempotent', () => {
  ...
  assert.deepEqual(first.applied, [
    ...
    '006_qualification_staffing',
    '007_organization_topology',
    '008_runtime_projection',
    '009_incidents_checkpoints',
    '010_maintenance',
    '011_runtime_launch_policy',
    '012_runtime_access_profiles',
    '013_provider_hub',
  ...
  assert.ok(tables.includes('v2_capability_definitions'));
  assert.ok(tables.includes('v2_capability_claims'));
  assert.ok(tables.includes('v2_requirement_sets'));
  assert.ok(tables.includes('v2_qualification_assessments'));
  assert.ok(tables.includes('v2_staffing_rules'));
  assert.ok(tables.includes('v2_staffing_constraints'));
  assert.ok(tables.includes('v2_role_definitions'));
  assert.ok(tables.includes('v2_position_templates'));
  assert.ok(tables.includes('v2_position_relations'));
  assert.ok(tables.includes('v2_runtime_sessions'));
  assert.ok(tables.includes('v2_runtime_session_edges'));
  assert.ok(tables.includes('v2_activity_events'));
  assert.ok(tables.includes('v2_execution_sync_runs'));
  assert.ok(tables.includes('v2_projection_checkpoints'));
  ...
  assert.ok(tables.includes('v2_runtime_access_profiles'));
  assert.ok(tables.includes('v2_provider_connections'));
  assert.ok(tables.includes('v2_profile_provider_links'));
  assert.ok(tables.includes('v2_provider_connection_attempts'));
});
