...
import type { ReviewCompletionEvidence, WorkspaceDescriptor } from '../src/v4/orchestration/contracts.js';

function git(cwd: string, args: string[]): string {
  return execFileSync('git', ['-c', 'commit.gpgsign=false', '-C', cwd, ...args], { encoding: 'utf8' }).trim();
}
...
function repository(root: string): { repositoryPath: string; rootRevision: string; baseRevision: string } {
...
function implementationEvidence(workspace: WorkspaceDescriptor, resultRevision: string, override: Record<string, unknown> = {}) {
  return {
    version: 1,
...
function reviewEvidence(workspace: WorkspaceDescriptor, reviewedSha: string, override: Partial<ReviewCompletionEvidence> = {}) {
  return {
    version: 1,
    executionId: workspace.executionId,
    phase: 'REVIEW',
...
test('LocalGitWorkspace observes allowed repositories and rejects roots and symlinks', async () => {
  const value = fixture();
  const observation = await value.adapter.observeRepository(value.repositoryPath, value.baseRevision);
  ...
  assert.equal((await value.adapter.observeRepository(value.repositoryPath, 'missing')).commitExists, false);
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof V4Error && error.code === 'WORKSPACE_REPOSITORY_NOT_ALLOWED',
  );
...
test('LocalGitWorkspace provisions an exact revision idempotently with durable provenance', async () => {
  ...
  const first = await value.adapter.provision({
    executionId: 'exec-provision', repositoryPath: value.repositoryPath, sourceRevision: value.baseRevision, phase: 'IMPLEMENT',
  });
  ...
  assert.ok(fs.existsSync(path.join(value.managedRoot, 'v4', 'executions', 'exec-provision', 'workspace.json')));
  await assert.rejects(
    () => value.adapter.provision({ executionId: 'exec-provision', repositoryPath: value.repositoryPath, sourceRevision: value.rootRevision }),
...
test('LocalGitWorkspace implementation verification rejects no-op, dirty and mismatched evidence then accepts exact committed work', async () => {
  ...
  const workspace = await value.adapter.provision({
    executionId: 'exec-implementation', repositoryPath: value.repositoryPath, sourceRevision: value.baseRevision, phase: 'IMPLEMENT',
  });
  await assert.rejects(
  ...
  await assert.rejects(
    () => value.adapter.verifyImplementation(workspace),
    (error: unknown) => error instanceof V4Error && error.code === 'WORKSPACE_IMPLEMENTATION_EVIDENCE_MISMATCH',
  );
  write(workspace.evidenceHostPath, JSON.stringify(implementationEvidence(workspace, resultRevision, {
    tests: [{ command: 'npm test', status: 'FAIL', exitCode: 1 }],
  })));
  await assert.rejects(
    () => value.adapter.verifyImplementation(workspace),
    (error: unknown) => error instanceof V4Error && error.code === 'WORKSPACE_IMPLEMENTATION_TEST_GATE_FAILED',
  );
  write(workspace.evidenceHostPath, JSON.stringify(implementationEvidence(workspace, resultRevision)));
...
test('LocalGitWorkspace provisions independent exact-SHA review snapshots and validates verdict evidence', async () => {
  ...
  const implementation = await value.adapter.provision({
    executionId: 'exec-review-source', repositoryPath: value.repositoryPath, sourceRevision: value.baseRevision, phase: 'IMPLEMENT',
  });
  ...
  const review = await value.adapter.provision({
    executionId: 'exec-review', repositoryPath: value.repositoryPath, sourceRevision: reviewedSha, phase: 'REVIEW', sourceWorkspace: implementation,
  });
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof V4Error && error.code === 'WORKSPACE_REVIEW_EVIDENCE_MISMATCH',
  );
  ...
  write(review.evidenceHostPath, JSON.stringify(reviewEvidence(review, reviewedSha, {
    verdict: 'FAIL', findings: ['blocking issue'], checks: [{ command: 'npm test', status: 'FAIL', exitCode: 1 }], summary: 'Rejected.',
  })));
...
test('LocalGitWorkspace integration rejects stale, dirty and non-descendant candidates and fast-forwards exact accepted revision', async () => {
  ...
  const candidate = await value.adapter.provision({
    executionId: 'exec-integrate', repositoryPath: value.repositoryPath, sourceRevision: value.baseRevision, phase: 'IMPLEMENT',
  });
  ...
  await assert.rejects(
    () => value.adapter.integrateAcceptedRevision({
      repositoryPath: value.repositoryPath, expectedRevision: value.rootRevision, acceptedRevision, candidateWorkspace: candidate,
    }),
    (error: unknown) => error instanceof V4Error && error.code === 'WORKSPACE_INTEGRATION_STALE_HEAD',
  );

  write(path.join(value.repositoryPath, 'dirty.txt'), 'dirty\n');
  await assert.rejects(
    () => value.adapter.integrateAcceptedRevision({
      repositoryPath: value.repositoryPath, expectedRevision: value.baseRevision, acceptedRevision, candidateWorkspace: candidate,
    }),
  ...
  const unrelated = await value.adapter.provision({
    executionId: 'exec-unrelated', repositoryPath: value.repositoryPath, sourceRevision: value.baseRevision, phase: 'IMPLEMENT',
  });
  configureWriter(unrelated);
  git(unrelated.hostPath, ['checkout', '--orphan', 'unrelated']);
  for (const entry of fs.readdirSync(unrelated.hostPath)) if (entry !== '.git') fs.rmSync(path.join(unrelated.hostPath, entry), { recursive: true, force: true });
  ...
  await assert.rejects(
    () => value.adapter.integrateAcceptedRevision({
      repositoryPath: value.repositoryPath, expectedRevision: value.baseRevision, acceptedRevision: unrelatedRevision, candidateWorkspace: unrelated,
    }),
    (error: unknown) => error instanceof V4Error && error.code === 'WORKSPACE_ACCEPTED_REVISION_NOT_DESCENDANT',
  );
