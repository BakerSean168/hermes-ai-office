...
Plan cards project the durable workflow state rather than guessing from batch names. Business progress excludes synthetic control-plane work, while `currentActivity` exposes integration repair, aggregate `BATCH_VERIFY`, delivery repair, post-merge repair, backend/model, attempt, revision, blocking reason, and the latest upstream `lastObservedAt` activity timestamp as a stable read-only dashboard contract. `lastObservedAt` is an observed worker/activity timestamp, not a claim that semantic progress was made. GitHub external-change plans additionally project the durable PR governance identity (repository/PR/producer/head refs, governed revision, and last published governance revision/status) without creating a second status machine in the dashboard.
