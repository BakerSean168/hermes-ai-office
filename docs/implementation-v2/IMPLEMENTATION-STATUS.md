...
`nico-free-deepseek` and the CPA technical channel still named `planner-pool` remain infrastructure evidence, not business Supplier identity. The latter still carries DeepSeek-compatible traffic for existing Hermes `provider=deepseek` profiles and must not be confused with the retired Planner Pool Supplier. Missing commercial evidence is displayed as unclassified rather than guessed. The personal CPA gateway is displayed as `My CPA`, and the retired `planner-pro` alias has been removed while the underlying `deepseek-v4-pro` model remains available.
