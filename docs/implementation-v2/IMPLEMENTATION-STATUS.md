...
Personal channels are now a separate account-pool projection rather than a synonym for Gateway inventory. Production currently exposes two account-pool sources: **My CPA / xAI-Grok** (imported xAI credentials, with Grok models discovered from CPA) and **Grok2API** (a local multi-account Grok Build / Console / Web gateway). Their projections include only safe account counts, readiness, health, and model identifiers; credentials never enter AI Office business storage.
