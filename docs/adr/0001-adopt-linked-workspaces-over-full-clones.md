...
Durable claim expiry is not used as the physical filesystem mutex. Every provision also holds a per-execution kernel `flock` in a service-private lock directory for the complete destructive/publication interval. This prevents a successor that took an aged SQLite claim from touching the same execution path until the previous process has stopped all filesystem mutation. The durable publication token is still renewed at critical boundaries and remains the authority for attach/fail semantics.
