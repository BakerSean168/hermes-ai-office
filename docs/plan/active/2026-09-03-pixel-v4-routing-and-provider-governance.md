...
| 5. Claude Opus + Claude Code exact-SHA review             | BLOCKED_BY_RESOURCE | Live `/api/v4/resources` and `/api/v4/runtime-admission` currently contain zero `claude-opus-* / claude-code-acp` bindings. Keep this row open until a healthy approved provider is bound; do not substitute Antigravity or fabricate review evidence.                                                                                                                                                                                                                                                             |
...
shadow selection comparison
  -> one project implementation canary
  -> independent review canary
  -> all trusted Pixel projects
```

During shadow mode, log the new selected profile next to the legacy route decision without changing the execution.
...
Cutover status: selector-authoritative production routing shipped in `e5b534f`. Production health must report `routingAuthority=RESOURCE_SELECTOR` with both compatibility route arrays empty; the release gate fails closed if dual routing authority reappears. Historical route-only executions remain readable, and selector-disabled startup remains the explicit rollback path.
