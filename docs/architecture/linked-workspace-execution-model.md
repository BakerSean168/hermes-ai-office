...
The control-plane systemd namespace must not bind-mount `MODEL_CP_V3_WORKSPACE_ROOT` separately with `ReadWritePaths`. Under `ProtectSystem=full`, `/opt` is already writable; a redundant bind mount changes the VFS mount identity and breaks hardlink/atomic-rename semantics even when `stat(2)` reports the same underlying device. The durable database directory may remain explicitly writable.
