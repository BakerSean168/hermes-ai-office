...
export function ModelWorkforcePanel() {
  ...
  const [v2Snapshot, setV2Snapshot] = useState<V2WorkforceProjection | null>(null);
  const [adminEnabled, setAdminEnabled] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const load = useCallback(async () => {
    const [legacyResult, configResult, v2Result] = await Promise.allSettled([
      fetch('/api/model/workforce'),
      fetch('/api/model/config'),
      fetch('/api/model/v2/workforce'),
    ]);
    const failures: string[] = [];

    if (legacyResult.status === 'fulfilled' && legacyResult.value.ok) {
      setSnapshot((await legacyResult.value.json()) as WorkforceSnapshot);
    } else {
      failures.push('legacy gateway projection unavailable');
    }

    if (v2Result.status === 'fulfilled' && v2Result.value.ok) {
      setV2Snapshot((await v2Result.value.json()) as V2WorkforceProjection);
    } else {
      failures.push('employee roster unavailable');
    }

    if (configResult.status === 'fulfilled' && configResult.value.ok) {
      const config = (await configResult.value.json()) as { adminEnabled?: boolean };
      setAdminEnabled(config.adminEnabled === true);
    }

    setError(failures.length === 2 ? failures.join(' · ') : null);
  }, []);

  useEffect(() => {
    void load();
    const legacySource = new EventSource('/api/model/events');
    const v2Source = new EventSource('/api/model/v2/events');
    legacySource.onmessage = () => void load();
    v2Source.onmessage = () => void load();
    legacySource.onerror = () =>
      setError((value) => value ?? 'Legacy gateway event stream reconnecting');
    v2Source.onerror = () => setError((value) => value ?? 'Employee event stream reconnecting');
    return () => {
      legacySource.close();
      v2Source.close();
    };
    ...
    <div className="pixel-panel p-5" style={{ backgroundColor: 'var(--color-bg)' }}>
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          ...
          <div className="text-xs text-text-muted">
            {v2Snapshot ? (
              <>
                {v2Snapshot.summary.employees} employee
                {v2Snapshot.summary.employees === 1 ? '' : 's'} · {v2Snapshot.summary.employed}{' '}
                employed · {v2Snapshot.summary.currentDuties} working now ·{' '}
                {compact(v2Snapshot.summary.requests)} requests
              </>
            ) : (
              <>
                Compatibility view · {snapshot?.channels.length ?? 0} channels ·{' '}
                {snapshot?.workers.length ?? 0} legacy workers
              </>
            )}
          </div>
        </div>
        <div className="text-right text-xs text-text-muted">
          <div>{adminEnabled ? 'management enabled' : 'read only'}</div>
          {error && <div>{error}</div>}
        </div>
      </div>
      ...
      {v2Snapshot && (
        <>
          ...
          >
            {v2Snapshot.employees.map((employee) => (
              <EmployeeCard employee={employee} key={employee.id} />
            ))}
          </div>

          {v2Snapshot.employees.length === 0 && (
          ...
          <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-text-muted">
            <span>
              Career usage: {compact(v2Snapshot.summary.inputTokens)} input ·{' '}
              {compact(v2Snapshot.summary.outputTokens)} output
            </span>
            <span>
              {money(v2Snapshot.summary.actualCost)} actual ·{' '}
              {money(v2Snapshot.summary.marketValue)} market value
            </span>
            {v2Snapshot.gateways.map((gateway) => (
              <span key={gateway.id}>
      ...
      {snapshot && (
        <details className="mt-5 border-t-2 border-border pt-3">
          <summary className="cursor-pointer text-sm font-semibold">
            Gateway compatibility and channel operations · {snapshot.channels.length} channels ·{' '}
            {snapshot.workers.length} legacy workers
          </summary>
          <div className="mt-3">
            ...
            >
              {snapshot.channels.map((channel) => (
                <ChannelCard
                  ...
                  contract={snapshot.contracts.find(
                    (contract) => contract.channelId === channel.id,
                  )}
                  prices={prices}
                  adminEnabled={adminEnabled}
                  onChanged={load}
